...
public class HotelReservationSystem {
    ...
    private static boolean isRoomReserved(Connection connection, int roomNumber) {
        try {
            String sql = "SELECT room_number FROM reservation WHERE room_number = " + roomNumber;

            try (Statement statement = connection.createStatement();
                 ResultSet resultSet = statement.executeQuery(sql)) {

                return resultSet.next(); // If there's a result, the room is already reserved
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private static void updateReservation(Connection connection, Scanner scanner) {
        try {
            ...
            // Check if the new room is already reserved
            if (isRoomReserved(connection, newRoomNumber)) {
                System.out.println("Room " + newRoomNumber + " is already reserved. Please choose another room.");
                return;
            }
